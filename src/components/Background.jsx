...
export default function Background({ enabled = true, reduceMotion = false }) {
  ...
  const pointerGlow = useMotionTemplate`radial-gradient(560px circle at ${glowX} ${glowY}, rgba(255,255,255,0.035), transparent 70%)`;

  if (!enabled) return <div className="fixed inset-0 -z-10 bg-[#07070a]" />;
    ...
    <div className="fixed inset-0 -z-10 overflow-hidden bg-[#07070a]">
      ...
      <motion.div style={active ? { x: driftX, y: driftY } : undefined} className="absolute inset-0">
        ...
        <motion.div
          ...
          style={{ background: "radial-gradient(circle,rgba(56,89,148,0.06),transparent 70%)" }}
